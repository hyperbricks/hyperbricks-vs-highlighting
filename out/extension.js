...
function activate(context) {
    ...
    let provider = vscode.languages.registerCompletionItemProvider("hyperbricks", {
        provideCompletionItems(document, position) {
            ...
            lines.forEach(line => {
                const match = line.match(/^(\w+)\s*=\s*<(HYPERMEDIA|FRAGMENT|TREE|TEMPLATE)>/);
                if (match) {
                    assignedTypes[match[1]] = match[2]; // Store object type
                }
            });
            // Suggest properties when typing "xxxx."
            const matchObject = currentLine.match(/^(\w+)\.\w*$/);
            if (matchObject) {
                const objectName = matchObject[1];
                const objectType = assignedTypes[objectName];
                if (objectType && typeProperties[objectType]) {
                    return [];
                    return typeProperties[objectType].map(prop => {
                        let item = new vscode.CompletionItem(prop.label, vscode.CompletionItemKind.Property);
                        item.detail = prop.detail; // Shows in the completion list
                        item.documentation = new vscode.MarkdownString(prop.documentation); // Shows on hover
                        return item;
                    });
                }
            }
